...
export class LoanOverviewPieComponent implements OnInit, AfterViewInit, OnDestroy, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (this.viewInitialized && changes['loanAccounts']) {
      this.createChart();
    }
  }
  ...
  private createChart(): void {
    ...
    if (!this.loanAccounts || !Array.isArray(this.loanAccounts) || this.loanAccounts.length === 0) {
      return;
    }

    // Check if canvas element exists
    const canvas = document.getElementById('loan-pie') as HTMLCanvasElement;
    if (!canvas) {
      console.warn('Canvas element not found for loan pie chart');
      return;
    }
    ...
    try {
      this.chart = new Chart(canvas, {
        ...
        data: {
          labels,
          datasets: [{
            backgroundColor: ['#FFCE56', '#36A2EB', '#4BC0C0', '#FF6384', '#9966FF', '#4BC0C0', '#FFCE56', '#36A2EB', '#FF6384'],
            data
          }]
        },
        options: {
      ...
      console.error('Failed to create loan pie chart:', error);
    }
  }
}
